...
- **D2 配置删除纯化**：`ExecuteDeleteTagAsync`（:2083）/`ExecuteDeleteGroupAsync`（:2127）重写为纯配置删除——**快捷键绑定引用拒绝前移**（删定义前读 `_settingsService.Load().Shortcuts` 判 `ApplyTag + TagId`，文案沿用 `TagService.DeleteTagAsync:76-80` 现文案；否则 `ValidateBindings` 会在 Save 时以「引用的标签不存在」错误文案拒绝，口径不符 A2）；保留 `TagFilterState.RemoveTagReferences` 摘树调用与 `ApplyTagFilter` 刷新。`TagService.DeleteTagAsync`/`DeleteGroupAsync` 连锁语义迁走后删除（绑定拒绝谓词随迁）；`RenameFilesAsync`（:2181）收窄为重命名专用（唯一残留调用方 `ExecuteRenameTagAsync`）。
...
- **D5 图库右栏宿主**：`MainWindow.xaml` 的 `ContentAreaGrid`（:240）内图库 Grid（`GalleryVisibility` 显隐那层）右缘叠加浮层（`Canvas.ZIndex=1`、`HorizontalAlignment=Right`）——随 GalleryVisibility 单图模式天然隐藏、位于工具栏行之下无需 TopChromeHeight 避让、不在 ChromeLayer 根不受 XamlCompiler 顺序坑影响。新控件 `Views\GallerySelectionPanelControl.xaml(.cs)`，双 Border 展开 280 / 折叠 36（仿 `SingleImageView` InfoPanelOverlay/CollapsedBar :56-263；宽度常量入 `SingleImageView.xaml.cs:44-47` 同族锚点注释）。**遮盖式布局铁律：不占布局槽、不改瀑布流几何。**
- **D6 并集重算策略**：全量重算（O(选中集 × 平均标签数) 内存字典运算），双挂点（选中变化 + 打标/移除收口），容忍 `SelectSingleCard`/`SelectCardRange` 先清后加的 0→n 中间态通知（重算幂等，中间态闪变可接受）。本期不做防抖/差量化（数万张选中预计 <100ms，实机走查验证，超预期再议）。
- **D7 批量目录三态**：`TagCatalogEntry` 的 `IsApplied` 二值改三态——`AppliedToAll`（选中集全部已含 → 禁点）/`AppliedToSome`（部分含 → 可点，再点幂等替换）/`None`。`TagCatalogDialog` 新增批量构造变体（快照 = 配置组 + 选中集并集 + 选中数），XAML 的 `Not(IsApplied)` 绑定与 `AppliedSuffix` 转换器同步改；单图口径构造保留不动。
...
- **D10 批量删除服务**：`IFileOperationService`/`FileOperationService`（Services\，Core 可测）新增 `DeleteToRecycleBin(IReadOnlyList<string> paths)` 批量变体，返回 `BatchOperationResult`（逐文件聚合失败，沿用 `TagService.RenameAll` 的 IOException/UnauthorizedAccessException 聚合模式）；`LibraryIndexService` 新增 `RemovePathsAsync(IReadOnlyList<string>)` 单事务批量删行（仿 `UpsertChunkAsync:73-83`）。
...
tests\SimpleViewer.Tests\TagProjectionTests.cs   # 新：T-PR 系列
tests\SimpleViewer.Tests\FileOperationServiceTests.cs # 新：T-FO 系列
tests\SimpleViewer.Tests\LibraryIndexServiceTests.cs  # 改：T-IX 续号
...
| `Views\TagSidebarControl.xaml(.cs)` | 未定义区插入滚动区尾、「＋ 新建标签组」(:271-276) 之前：区标题「未定义标签」+ WrapPanel chip 流 + chip 主点击弹 MenuFlyout（✕ 删除 / ⇱ 收纳）+ MaxHeight 限高滚动 + 空集合整区 Collapsed（E1） | 组模板/行模板零触碰（tag-group-tree-ui 拍板守护） |
...
| `tests\...` | 见测试策略 | T_TG_05:118、T_ST_05:289 调整 |

## 详细实现步骤

- Step 1 — phase-core-projection — blocking: yes — qa: auto：新建 `Services\TagProjection.cs`（两个纯函数族 + POCO 结果类型）与 `tests\SimpleViewer.Tests\TagProjectionTests.cs`（T-PR1~T-PR8）；`dotnet test` 全绿。
...
- Step 7 — phase-delete-selection — blocking: yes — qa: auto + manual_user：`FileOperationService.DeleteToRecycleBin` 批量 + `FileOperationServiceTests`（T-FO1~3）；`LibraryIndexService.RemovePathsAsync` + T-IX 续号；`DeleteSelectionCommand`（确认→批量删除→锁内 `_galleryItems` 移除→索引批量删→`ScanStatusText` 刷新→`ApplyTagFilter()`（内含选中清空/命中数/侧栏重建）→`_imageFiles` 裁剪）；`DispatchShortcut` DeleteImage 分流；防重入闸（仿 `_isTagOperationRunning`）。
...
- Step 9 — phase-verify-full — blocking: yes — qa: auto + manual_user：`scripts\build.ps1` + `dotnet test tests\SimpleViewer.Tests\SimpleViewer.Tests.csproj` 全绿门禁；实机走查清单（manual_user）：A1/A3 删除定义 0 改名+未定义区出现、B1/B2/B3 未定义区三操作、C1~C4 右栏、D1~D4 工具栏与删除、E1 空区隐藏、双主题、大选中集（Ctrl+A 全选）体感、回归（单图目录打标、单图 Delete、拖拽打标、Esc 三态、Ctrl+A、筛选面板打开时快捷键抑制）。
...
### 测试用例（新前缀 T-PR / T-FO，T-IX 续号——已核对未与 13 个占用缩写冲突）

- T-PR1 — blocking: yes — 未定义集合 = counts 键集 − 配置组标签集（OrdinalIgnoreCase 差集，配置组大小写变体正确排除）
- T-PR2 — blocking: yes — 计数保留计数键值；排序 = 计数降序、同计数按名 Ordinal
- T-PR3 — blocking: yes — 全部标签已配置 / counts 空 → 空列表（E1 数据层）
- T-PR4 — blocking: yes — 拼写口径：同名多拼写以计数键（文件名侧首个聚合拼写）为准，不取配置侧拼写
- T-PR5 — blocking: yes — 选中集并集计算 + 各标签选中集内计数（如 30 张中 18 张含 Z → 「Z 18」）
- T-PR6 — blocking: yes — 并集含未定义标签（配置组外标签不滤除——C2 数据层）
- T-PR7 — blocking: yes — 空选中 → 空并集
- T-PR8 — blocking: yes — 并集大小写合并口径（OrdinalIgnoreCase 同名合并计数，拼写取首见）
- T-FO1 — blocking: yes — 批量回收站删除：N 文件成功、返回成功计数、文件不存在于原路径
- T-FO2 — blocking: yes — 部分失败聚合（锁定句柄文件失败进 Failures、其余成功不回滚）
- T-FO3 — blocking: yes — 空路径列表 → no-op 成功
- T-IX13 — blocking: yes — RemovePathsAsync 单事务批量删行，查询确认全删
- 现有调整：T_TG_05（DeleteTagAsync 连锁 → RemoveTagAsync 按名移除口径）；T_ST_05（绑定拒绝文件不动 → 注记拒绝点迁 VM，服务层锁定 ValidateBindings 悬空拒绝不变）

映射：T-PR1~4→Step 1/3；T-PR5~8→Step 1/4；T-FO→Step 7；T-IX13→Step 7；GWT 验收 A1/A2/A3/A4→Step 2/8（A2 拒绝为 VM 层，走查+代码审查覆盖）；B1~B3→Step 3；C1~C4→Step 4/5；D1~D4→Step 6/7；E1→Step 3。
...
- XAML 硬约束全家桶：禁 FontIcon/SymbolIcon、中文 XAML UTF-8 BOM、Click 不用 Tapped、修饰键只判 Down、MainInfoBar 在 MainAreaGrid 之后、自绘按钮 UseSystemFocusVisuals=false、ScrollViewer overlay 右 Padding 让位。
