...
- `release\SimpleViewer-v1.1.0-win-x64.zip` 解压出 **264 个文件**、`viewer.exe` / `MainWindow.xbf` 齐备；
  对**解压后的 exe** 直接跑一轮屏外 soak：图库缩略图渲染、UIA 滚动、侧栏筛选点击、进程存活、
  事件日志无新崩溃、`brushNew=0`、位图池 11-11 —— 与 publish 目录、本机安装三处同源同结果。

## 7. 跨版本像素对照（1.6 构建 vs 2.5.1 构建）

方法：同一夹具图库与设置（主题、LastLibraryRoot 一致），两个构建各自启动 → 窗口移到屏幕外同一位置
`(4560,40) 1600×900` → `PrintWindow` 连拍，两帧相同才算安定，再比对安定帧；每主题额外做一次
**同二进制复跑**作方法对照（暴露运行间噪声，缺它会把噪声误读成版本差异）。

| 主题 | 对照（1.6 同二进制两次运行） | 1.6 vs 2.5.1 |
|------|------------------------------|--------------|
| Dark | 左栏 0/378000、工具栏 0/144000、工作区 19616/947700（缩略图落位噪声） | 全窗 **23** 像素差（1:1 全采样 1,440,000）、左栏 22/378000、工具栏 0/144000、工作区 1/947700，**单通道最大差 5/255** |
| Light | 全窗 30/1440000（maxΔ1）、左栏 0 | 全窗 **46**/1440000（0.003%）、左栏 15/378000、工具栏 0/144000、工作区 31/947700，**单通道最大差 3/255** |

差异分布（Dark 逐像素定位）：全部落在文字/字形抗锯齿边缘（如 (150,203)(154,206)(192,450) 一带，颜色差 1–5），
无任何结构性或成片差异；工具栏横条两主题均**逐位相同**。

结论：跨版本差异处于「字形抗锯齿/亚像素取整」量级，且**同二进制复跑自身的噪声同量级**（30 像素）——
**无可感知的 UI 变化**。旧 1.6 构建保留在 `E:\App\Others\viewer.old-1.0.6`，既作此对照的基线也可回退。
