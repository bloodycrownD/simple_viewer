...
- （2026-09-19 CR 修订补记）settings v2 后续增补 `LastLibraryRoot` 字段：启动自动恢复上次图库根目录（用户拍板保留，增量能力回写见 PRD 需求 4）；旧配置缺失该字段容忍为空（不自动恢复，行为同无该字段时）。
...
- Step 9 — phase-tag-sidebar — blocking: yes — qa: manual_user：`TagSidebarControl`（分组 chip、互斥组单选圆点样式、每标签计数实时刷新、点击=筛选；解析到但不属于任何已配置组的标签进入左栏固定末位的**"未分组"虚拟组**——可点击筛选、不可配置互斥属性；为"未分组"组打标属普通非互斥操作；如需把未分组标签纳入正式组，在正式组新建同名单标签即视为同一平铺字符串（文件名协议的平铺语义，无需专门的"移动"操作））+ `TagEditDialog`（新建/重命名/删除组与标签，重命名/删除前显示影响张数）。走查 M3。**【2026-09-19 CR 修订："未分组"虚拟组已移除——聚合 UI 忽略无组标签（不进侧栏/计数/角标），清理出口 = 详情页 chips ✕ 逐图移除、同名收编口径保留，见 features/ungrouped-tags-ignore 与 cr-fix-spec；点击=筛选的心智同步调整为无修饰单选重置 + Ctrl 加减选，见 features/tag-filter-select-model】**
- Step 10 — phase-batch — blocking: yes — qa: manual_user：瀑布流多选（单击/Ctrl/Shift 连选/Ctrl+A）+ 点击左栏标签批量打标/Shift 移除 + 单图模式当前图打标 + `InfoBar` 进度与失败回执（成功数/失败数与原因，成功不回滚）。走查 M4（含 Esc 清空选中、单张打标操作反馈 ≤1s）。**【2026-09-19 CR 修订：批量进度保留，全部成功静默不弹（就地反馈已充分），仅失败/部分失败弹终态回执含明细；批量移除入口不做——移除走详情页 ✕ 逐图/同名收编，用户拍板，见 cr-fix-spec】**
- Step 11 — phase-filter — blocking: yes — qa: manual_user：筛选条（激活标签 chip、OR 语义标识、命中数、单删/清空）→ `LibraryIndexService.QueryByTags` 驱动瀑布流刷新，筛选态下单图翻页在命中集内循环。走查 M5。**【2026-09-19 CR 修订：清空按钮已移除——取消筛选 = 二次点击标签 / chip ✕ / 无标签按钮二次点击；同轮增补"无标签筛选"∅ 入口（与标签筛选互斥），见 features/untagged-filter-entry 与 cr-fix-spec】**
