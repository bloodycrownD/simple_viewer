...
- **版本与 tag 规则（2026-09-24 用户拍板）**：tag 是不可变发布物——同一版本号反复强推重打（v1.0.2 曾九连推）毫无意义。每轮修复收口要出新包时**一律递增 patch（v1.0.3、v1.0.4 …）**：CHANGELOG 为该轮开新版本节、打新 tag、push 触发 CI 出新 draft；上一轮未 Publish 的旧 draft 提醒用户在 Releases 页删除（勿堆积多个 draft）。
